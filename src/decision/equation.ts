...
import { IsImplied, LinearEquation } from './smith';


/**
 * Checks that consecutive formulas in a chain are connected: the right side of
 * each formula equals the left side of the next. Returns an error message if
 * not, or undefined if valid.
 */
export function IsChainConnected(steps: Formula[]): string | undefined {
  for (let i = 0; i + 1 < steps.length; i++) {
    if (!steps[i].right.equals(steps[i + 1].left)) {
      return `step ${i + 1} ends with ${steps[i].right.to_string()} but step ${i + 2} starts with ${steps[i + 1].left.to_string()}`;
    }
  }
  return undefined;
}
...
/**
 * Validates that a chain of formulas constitutes a valid equation proof.
 * Every step must use "=". Returns an error message or undefined if valid.
 */
export function IsEquationChainValid(steps: Formula[]): string | undefined {
...
export function IsEquationImplied(eqs: Formula[], eq: Formula): boolean {
  ...
  const eqs_sides = eqs.map(
      (eqn) => [eqn.left.normalize(), eqn.right.normalize()]);
  const eq_sides = [eq.left.normalize(), eq.right.normalize()];
