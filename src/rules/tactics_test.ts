...
describe('AlgebraTactic with inequality', function() {
...
describe('DefinitionTactic edge cases', function() {
  ...
  it('backward defof unconditional rejects when knowns provided', function() {
    ...
    assert.throws(() => CreateTactic(ast, goal, env), /must not be provided/);
  });

});


describe('ApplyTactic edge cases', function() {
  ...
  it('backward apply rejects when premise not cited', function() {
    ...
    assert.throws(() => CreateTactic(ast, ParseExpr('x'), env), /premise.*must be provided/);
  });
  ...
  it('backward apply rejects when no premise but facts cited', function() {
    ...
    const env = new TopLevelEnv([], [], [ParseFormula('x = 1')], [comm]);
    const ast = ParseBackwardRule('apply comm since 1');
    assert.throws(() => CreateTactic(ast, ParseExpr('y + x'), env), /no premise.*must not be provided/);
  });
  ...
  it('backward apply rejects theorem with non-atomic conclusion', function() {
    ...
    assert.throws(() => CreateTactic(ast, ParseExpr('x'), env), /non-atomic conclusion/);
  });

  it('backward apply with non-atom premise ignores it in atomPremises', function() {
    const thm = new TheoremAst('foo', [['n', 'Int']],
        [new NotProp(ParseFormula('n = 0'))],
