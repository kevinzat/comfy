...
describe('checkFuncDef', function() {
  ...
  it('throws TypeMismatchError for invariant with non-Int operand in < comparison', function() {
    const env = makeEnvWithList();
    const func = new FuncDef('Int', 'f', [new Param('List', 'xs')], [
      new WhileStmt(new RelAst(Constant.of(0n), '!=', Constant.of(0n)),
          [new RelAst(Variable.of('xs'), '<', Constant.of(0n))],
          [new PassStmt()]),
      new PassStmt(),
    ]);
    assert.throws(() => checkFuncDef(env, func), TypeMismatchError);
  ...
  it('throws UnknownNameError for invariant referencing unknown variable', function() {
    ...
    const func = new FuncDef('Int', 'f', [], [
      new WhileStmt(new RelAst(Constant.of(0n), '!=', Constant.of(0n)),
          [new RelAst(Variable.of('z'), '>=', Constant.of(0n))],
          [new PassStmt()]),
      new PassStmt(),
    ]);
  ...
  it('throws TypeMismatchError for < when both operands are non-Int', function() {
    ...
    const inv = [new RelAst(Constant.of(0n), '>=', Constant.of(0n))];
    const func = new FuncDef('Int', 'f', [new Param('List', 'xs'), new Param('List', 'ys')], [
      new WhileStmt(new RelAst(Variable.of('xs'), '<', Variable.of('ys')), inv, [new PassStmt()]),
      new PassStmt(),
    ]);
    assert.throws(() => checkFuncDef(env, func), TypeMismatchError);
  });
  ...
  it('throws TypeMismatchError for < with non-Int left operand', function() {
    ...
    const inv = [new RelAst(Constant.of(0n), '>=', Constant.of(0n))];
    const func = new FuncDef('Int', 'f', [new Param('List', 'xs')], [
      new WhileStmt(new RelAst(Variable.of('xs'), '<', Constant.of(0n)), inv, [new PassStmt()]),
      new PassStmt(),
    ]);
    assert.throws(() => checkFuncDef(env, func), TypeMismatchError);
  });
  ...
  it('throws TypeMismatchError for < with non-Int right operand', function() {
    ...
    const inv = [new RelAst(Constant.of(0n), '>=', Constant.of(0n))];
    const func = new FuncDef('Int', 'f', [new Param('List', 'xs')], [
      new WhileStmt(new RelAst(Constant.of(0n), '<', Variable.of('xs')), inv, [new PassStmt()]),
      new PassStmt(),
    ]);
    assert.throws(() => checkFuncDef(env, func), TypeMismatchError);
  });
  ...
  it('rv has the return type of the function in ensures', function() {
    ...
    const func = new FuncDef('List', 'f', [new Param('List', 'xs')], [],
        [], [new RelAst(Variable.of('rv'), '<', Constant.of(0n))]);
    assert.throws(() => checkFuncDef(env, func), TypeMismatchError);
  });
  ...
  it('accepts true as a requires condition', function() {
    const env = makeEnv();
    assert.doesNotThrow(() => checkFuncDef(env, parse(`Int f(Int x) requires true { return x; }`)));
  });

  it('accepts false as a requires condition', function() {
    const env = makeEnv();
    assert.doesNotThrow(() => checkFuncDef(env, parse(`Int f(Int x) requires false { return x; }`)));
  });

  it('accepts true as an if condition', function() {
    const env = makeEnv();
    assert.doesNotThrow(() => checkFuncDef(env, parse(
        `Int f() { if (true) { return 1; } else { return 0; } }`)));
  });

  it('accepts false as a while condition', function() {
    const env = makeEnv();
    assert.doesNotThrow(() => checkFuncDef(env, parse(
        `Int f() { while (false) invariant true { pass; } return 0; }`)));
  });

  it('accepts not true as a condition', function() {
    const env = makeEnv();
    assert.doesNotThrow(() => checkFuncDef(env, parse(
        `Int f() { if (not true) { return 1; } else { return 0; } }`)));
  });
